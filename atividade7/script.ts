...
// 1. Criando a Interface (Molde exato do Objeto)
interface Produto {
    nome: string;
    preco: number;
    categoria: string;
...
const divLista = document.getElementById('listaProdutos') as HTMLDivElement;
const selectCategoria = document.getElementById('filtroCategoria') as HTMLSelectElement;
const btnSemEstoque = document.getElementById('btnSemEstoque') as HTMLButtonElement;
...
function renderizarProdutos(lista: Produto[]): void {
    ...
    for (let produto of lista) {
        let statusEstoque: string = produto.estoque > 0 
...
selectCategoria.addEventListener('change', function(evento: Event) {
